...
export class SendWeeklyInventoryFailureEmailJob extends Job<{}> {
  async perform(): Promise<void> {
    await findActiveBillingSchedulesInBatches(async (batch) => {
      this.logger.info(
        `Scheduling SendWeeklyInventoryFailureEmailJob for ${batch.length} shops`,
      );
      ...
      const results = await Promise.allSettled(
        batch.map(async (billingSchedule) => {
          ...
          if (isWeeklyNotificationFrequency) {
            ...
            this.logger.info(
              {params},
              'Scheduling SendWeeklyInventoryFailureEmailJob with params',
            );
      ...
      this.logger.info(
        ...
        'Successfully enqueued SendWeeklyInventoryFailureEmailJob jobs',
      );
    });
  }
}
